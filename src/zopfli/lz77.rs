...
extern "C" {
    ...
    #[no_mangle]
    fn ZopfliSublenToCache(sublen: *const libc::c_ushort, pos: size_t,
                           length: size_t, lmc: *mut ZopfliLongestMatchCache);
    /* Extracts sublen array from the cache. */
    #[no_mangle]
    fn ZopfliCacheToSublen(lmc: *const ZopfliLongestMatchCache, pos: size_t,
                           length: size_t, sublen: *mut libc::c_ushort);
    /* Returns the length up to which could be stored in the cache. */
    #[no_mangle]
    fn ZopfliMaxCachedSublen(lmc: *const ZopfliLongestMatchCache, pos: size_t,
                             length: size_t) -> libc::c_uint;
...
*/
#[no_mangle]
pub unsafe extern "C" fn ZopfliInitLZ77Store(mut data: *const libc::c_uchar,
                                             mut store:
                                                 *mut ZopfliLZ77Store) {
...
#[no_mangle]
pub unsafe extern "C" fn ZopfliCleanLZ77Store(mut store:
                                                  *mut ZopfliLZ77Store) {
...
unsafe extern "C" fn CeilDiv(mut a: size_t, mut b: size_t) -> size_t {
    return a.wrapping_add(b).wrapping_sub(1 as libc::c_int as
                                              libc::c_ulong).wrapping_div(b);
}
#[no_mangle]
pub unsafe extern "C" fn ZopfliCopyLZ77Store(mut source:
                                                 *const ZopfliLZ77Store,
                                             mut dest: *mut ZopfliLZ77Store) {
    let mut i: size_t = 0;
    let mut llsize: size_t =
        (288 as libc::c_int as
             libc::c_ulong).wrapping_mul(CeilDiv((*source).size,
                                                 288 as libc::c_int as
                                                     size_t));
    ...
    (*dest).litlens =
        malloc((::std::mem::size_of::<libc::c_ushort>() as
                    libc::c_ulong).wrapping_mul((*source).size)) as
            *mut libc::c_ushort;
    (*dest).dists =
        malloc((::std::mem::size_of::<libc::c_ushort>() as
                    libc::c_ulong).wrapping_mul((*source).size)) as
    ...
    (*dest).ll_counts =
        malloc((::std::mem::size_of::<size_t>() as
                    libc::c_ulong).wrapping_mul(llsize)) as *mut size_t;
    (*dest).d_counts =
        malloc((::std::mem::size_of::<size_t>() as
                    libc::c_ulong).wrapping_mul(dsize)) as *mut size_t;
    /* Allocation failed. */
    if (*dest).litlens.is_null() || (*dest).dists.is_null() {
        exit(-(1 as libc::c_int));
    }
    if (*dest).pos.is_null() { exit(-(1 as libc::c_int)); }
    ...
    while i < (*source).size {
        *(*dest).litlens.offset(i as isize) =
            *(*source).litlens.offset(i as isize);
        *(*dest).dists.offset(i as isize) =
            *(*source).dists.offset(i as isize);
        *(*dest).pos.offset(i as isize) = *(*source).pos.offset(i as isize);
        *(*dest).ll_symbol.offset(i as isize) =
            *(*source).ll_symbol.offset(i as isize);
        *(*dest).d_symbol.offset(i as isize) =
            *(*source).d_symbol.offset(i as isize);
        i = i.wrapping_add(1)
    }
    i = 0 as libc::c_int as size_t;
    while i < llsize {
        *(*dest).ll_counts.offset(i as isize) =
            *(*source).ll_counts.offset(i as isize);
        i = i.wrapping_add(1)
    }
    i = 0 as libc::c_int as size_t;
    while i < dsize {
        *(*dest).d_counts.offset(i as isize) =
            *(*source).d_counts.offset(i as isize);
        i = i.wrapping_add(1)
    };
}
/*
...
*/
#[no_mangle]
pub unsafe extern "C" fn ZopfliStoreLitLenDist(mut length: libc::c_ushort,
    ...
    let mut llstart: size_t =
        (288 as libc::c_int as
             libc::c_ulong).wrapping_mul(origsize.wrapping_div(288 as
                                                                   libc::c_int
                                                                   as
                                                                   libc::c_ulong));
    let mut dstart: size_t =
        (32 as libc::c_int as
             libc::c_ulong).wrapping_mul(origsize.wrapping_div(32 as
                                                                   libc::c_int
                                                                   as
                                                                   libc::c_ulong));
  ...
  each to save memory. */
    if origsize.wrapping_rem(288 as libc::c_int as libc::c_ulong) ==
        ...
        while i < 288 as libc::c_int as libc::c_ulong {
            if llsize & llsize.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                   == 0 {
                (*store).ll_counts =
                    if llsize == 0 as libc::c_int as libc::c_ulong {
                        malloc(::std::mem::size_of::<size_t>() as
                                   libc::c_ulong)
                    } else {
                        realloc((*store).ll_counts as *mut libc::c_void,
                                llsize.wrapping_mul(2 as libc::c_int as
            ...
            *(*store).ll_counts.offset(llsize as isize) =
                if origsize == 0 as libc::c_int as libc::c_ulong {
                    ...
                    *(*store).ll_counts.offset(origsize.wrapping_sub(288 as
                                                                         libc::c_int
                                                                         as
    ...
    if origsize.wrapping_rem(32 as libc::c_int as libc::c_ulong) ==
        ...
        while i < 32 as libc::c_int as libc::c_ulong {
            if dsize & dsize.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                   == 0 {
                (*store).d_counts =
                    if dsize == 0 as libc::c_int as libc::c_ulong {
                        ...
                        realloc((*store).d_counts as *mut libc::c_void,
                                dsize.wrapping_mul(2 as libc::c_int as
                                                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<size_t>()
                                                                                       as
                                                                                       libc::c_ulong))
                    } as *mut size_t
            ...
            *(*store).d_counts.offset(dsize as isize) =
                if origsize == 0 as libc::c_int as libc::c_ulong {
                    ...
                    *(*store).d_counts.offset(origsize.wrapping_sub(32 as
                                                                        libc::c_int
                                                                        as
                                                                        libc::c_ulong).wrapping_add(i)
                                                  as isize)
    ...
    if (*store).size &
       ...
       {
        (*store).litlens =
            if (*store).size == 0 as libc::c_int as libc::c_ulong {
                malloc(::std::mem::size_of::<libc::c_ushort>() as
                           libc::c_ulong)
            } else {
                realloc((*store).litlens as *mut libc::c_void,
                        (*store).size.wrapping_mul(2 as libc::c_int as
           ...
           (*store).size.wrapping_sub(1 as libc::c_int as libc::c_ulong) == 0
       {
        (*store).dists =
            if (*store).size == 0 as libc::c_int as libc::c_ulong {
                ...
                realloc((*store).dists as *mut libc::c_void,
                        (*store).size.wrapping_mul(2 as libc::c_int as
                                                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_ushort>()
                                                                                       as
                                                                                       libc::c_ulong))
           ...
           (*store).size.wrapping_sub(1 as libc::c_int as libc::c_ulong) == 0
       {
        (*store).pos =
            if (*store).size == 0 as libc::c_int as libc::c_ulong {
                ...
                realloc((*store).pos as *mut libc::c_void,
                        (*store).size.wrapping_mul(2 as libc::c_int as
                                                       libc::c_ulong).wrapping_mul(::std::mem::size_of::<size_t>()
                                                                                       as
                                                                                       libc::c_ulong))
    ...
    *(*store).pos.offset((*store).size as isize) = pos;
    ...
    if dist as libc::c_int == 0 as libc::c_int {
        (*store).size = origsize;
        if (*store).size &
               (*store).size.wrapping_sub(1 as libc::c_int as libc::c_ulong)
               == 0 {
            (*store).ll_symbol =
                if (*store).size == 0 as libc::c_int as libc::c_ulong {
                    malloc(::std::mem::size_of::<libc::c_ushort>() as
                    ...
                    realloc((*store).ll_symbol as *mut libc::c_void,
                            (*store).size.wrapping_mul(2 as libc::c_int as
                                                           libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_ushort>()
                                                                                           as
                                                                                           libc::c_ulong))
        ...
        if (*store).size &
            ...
            (*store).d_symbol =
                if (*store).size == 0 as libc::c_int as libc::c_ulong {
                    ...
                    realloc((*store).d_symbol as *mut libc::c_void,
                            (*store).size.wrapping_mul(2 as libc::c_int as
                                                           libc::c_ulong).wrapping_mul(::std::mem::size_of::<libc::c_ushort>()
                                                                                           as
                                                                                           libc::c_ulong))
        ...
        *(*store).d_symbol.offset((*store).size as isize) =
        ...
        (*store).size = (*store).size.wrapping_add(1);
        let ref mut fresh0 =
            *(*store).ll_counts.offset(llstart.wrapping_add(length as
                                                                libc::c_ulong)
                                           as isize);
        ...
        *(*store).d_symbol.offset((*store).size as isize) =
        ...
        (*store).size = (*store).size.wrapping_add(1);
        let ref mut fresh1 =
            *(*store).ll_counts.offset(llstart.wrapping_add(ZopfliGetLengthSymbol(length
                                                                                      as
                                                                                      libc::c_int)
                                                                as
                                                                libc::c_ulong)
        ...
        *fresh1 = (*fresh1).wrapping_add(1);
        let ref mut fresh2 =
            *(*store).d_counts.offset(dstart.wrapping_add(ZopfliGetDistSymbol(dist
                                                                                  as
                                                                                  libc::c_int)
                                                              as
                                                              libc::c_ulong)
...
#[no_mangle]
pub unsafe extern "C" fn ZopfliAppendLZ77Store(mut store:
                                                   *const ZopfliLZ77Store,
                                               mut target:
                                                   *mut ZopfliLZ77Store) {
    let mut i: size_t = 0;
    i = 0 as libc::c_int as size_t;
    while i < (*store).size {
        ZopfliStoreLitLenDist(*(*store).litlens.offset(i as isize),
                              *(*store).dists.offset(i as isize),
                              *(*store).pos.offset(i as isize), target);
        i = i.wrapping_add(1)
    };
}
#[no_mangle]
pub unsafe extern "C" fn ZopfliLZ77GetByteRange(mut lz77:
                                                    *const ZopfliLZ77Store,
                                                mut lstart: size_t,
                                                mut lend: size_t) -> size_t {
    let mut l: size_t = lend.wrapping_sub(1 as libc::c_int as libc::c_ulong);
    if lstart == lend { return 0 as libc::c_int as size_t }
    return (*(*lz77).pos.offset(l as
                                    isize)).wrapping_add((if *(*lz77).dists.offset(l
                                                                                       as
                                                                                       isize)
                                                                 as
                                                                 libc::c_int
                                                              ...
                                                              *(*lz77).litlens.offset(l
                                                                                          as
                                                                                          isize)
                                                                  as
                                                                  libc::c_int
                                                          }) as
                                                             libc::c_ulong).wrapping_sub(*(*lz77).pos.offset(lstart
                                                                                                                 as
                                                                                                                 isize));
}
unsafe extern "C" fn ZopfliLZ77GetHistogramAt(mut lz77:
                                                  *const ZopfliLZ77Store,
                                              mut lpos: size_t,
  ...
  all superfluous values of this chunk subtracted. */
    let mut llpos: size_t =
        (288 as libc::c_int as
             libc::c_ulong).wrapping_mul(lpos.wrapping_div(288 as libc::c_int
                                                               as
                                                               libc::c_ulong));
    let mut dpos: size_t =
        (32 as libc::c_int as
             libc::c_ulong).wrapping_mul(lpos.wrapping_div(32 as libc::c_int
                                                               as
                                                               libc::c_ulong));
    let mut i: size_t = 0;
    i = 0 as libc::c_int as size_t;
    ...
    while i < llpos.wrapping_add(288 as libc::c_int as libc::c_ulong) &&
              i < (*lz77).size {
        let ref mut fresh3 =
            *ll_counts.offset(*(*lz77).ll_symbol.offset(i as isize) as isize);
    ...
    i = lpos.wrapping_add(1 as libc::c_int as libc::c_ulong);
    while i < dpos.wrapping_add(32 as libc::c_int as libc::c_ulong) &&
              i < (*lz77).size {
        if *(*lz77).dists.offset(i as isize) as libc::c_int !=
               0 as libc::c_int {
            let ref mut fresh4 =
                *d_counts.offset(*(*lz77).d_symbol.offset(i as isize) as
                                     isize);
...
#[no_mangle]
pub unsafe extern "C" fn ZopfliLZ77GetHistogram(mut lz77:
                                                    *const ZopfliLZ77Store,
                                                mut lstart: size_t,
                                                mut lend: size_t,
                                                mut ll_counts: *mut size_t,
    ...
    if lstart.wrapping_add((288 as libc::c_int * 3 as libc::c_int) as
                               libc::c_ulong) > lend {
        memset(ll_counts as *mut libc::c_void, 0 as libc::c_int,
               (::std::mem::size_of::<size_t>() as
                    libc::c_ulong).wrapping_mul(288 as libc::c_int as
                                                    libc::c_ulong));
        memset(d_counts as *mut libc::c_void, 0 as libc::c_int,
               (::std::mem::size_of::<size_t>() as
        ...
        while i < lend {
            let ref mut fresh5 =
                *ll_counts.offset(*(*lz77).ll_symbol.offset(i as isize) as
                                      isize);
    ...
    histogram for this range. */
        ZopfliLZ77GetHistogramAt(lz77,
                                 lend.wrapping_sub(1 as libc::c_int as
                                                       libc::c_ulong),
                                 ll_counts, d_counts);
        if lstart > 0 as libc::c_int as libc::c_ulong {
            ...
            ZopfliLZ77GetHistogramAt(lz77,
                                     lstart.wrapping_sub(1 as libc::c_int as
                                                             libc::c_ulong),
            ...
            while i < 288 as libc::c_int as libc::c_ulong {
                ...
                *fresh7 =
                    (*fresh7 as
                         libc::c_ulong).wrapping_sub(ll_counts2[i as usize])
                        as size_t as size_t;
                i = i.wrapping_add(1)
            }
            i = 0 as libc::c_int as size_t;
            while i < 32 as libc::c_int as libc::c_ulong {
                ...
                *fresh8 =
                    (*fresh8 as
                         libc::c_ulong).wrapping_sub(d_counts2[i as usize]) as
                        size_t as size_t;
                i = i.wrapping_add(1)
            }
        }
...
#[no_mangle]
pub unsafe extern "C" fn ZopfliInitBlockState(mut options:
                                                  *const ZopfliOptions,
    ...
    if add_lmc != 0 {
        (*s).lmc =
            malloc(::std::mem::size_of::<ZopfliLongestMatchCache>() as
                       libc::c_ulong) as *mut ZopfliLongestMatchCache;
        ZopfliInitCache(blockend.wrapping_sub(blockstart), (*s).lmc);
    } else { (*s).lmc = 0 as *mut ZopfliLongestMatchCache };
}
#[no_mangle]
...
*/
unsafe extern "C" fn GetLengthScore(mut length: libc::c_int,
...
#[no_mangle]
pub unsafe extern "C" fn ZopfliVerifyLenDist(mut data: *const libc::c_uchar,
                                             mut datasize: size_t,
                                             mut pos: size_t,
                                             mut dist: libc::c_ushort,
    ...
    while i < length as libc::c_ulong {
        if *data.offset(pos.wrapping_sub(dist as
...
*/
unsafe extern "C" fn GetMatch(mut scan: *const libc::c_uchar,
 ...
 -> *const libc::c_uchar {
    if ::std::mem::size_of::<size_t>() as libc::c_ulong ==
        ...
        while scan < safe_end &&
                  *(scan as *mut size_t) == *(match_0 as *mut size_t) {
        ...
        while scan < safe_end &&
                  *(scan as *mut libc::c_uint) ==
                      *(match_0 as *mut libc::c_uint) {
...
*/
unsafe extern "C" fn TryGetFromLongestMatchCache(mut s: *mut ZopfliBlockState,
                                                 mut pos: size_t,
 ...
 -> libc::c_int {
    ...
    let mut cache_available: libc::c_uchar =
        (!(*s).lmc.is_null() &&
    ...
    if !(*s).lmc.is_null() && limit_ok_for_cache as libc::c_int != 0 &&
        ...
        if sublen.is_null() ||
            ...
            *length = *(*(*s).lmc).length.offset(lmcpos as isize);
            ...
            if !sublen.is_null() {
                ZopfliCacheToSublen((*s).lmc, lmcpos, *length as size_t,
                                    sublen);
                *distance = *sublen.offset(*length as isize);
                if *limit == 258 as libc::c_int as libc::c_ulong &&
                    ...
                    if *sublen.offset(*length as isize) as libc::c_int ==
                           *(*(*s).lmc).dist.offset(lmcpos as isize) as
                               libc::c_int {
                    } else {
            ...
            } else { *distance = *(*(*s).lmc).dist.offset(lmcpos as isize) }
...
possible.
*/
unsafe extern "C" fn StoreInLongestMatchCache(mut s: *mut ZopfliBlockState,
                                              mut pos: size_t,
                                              mut limit: size_t,
                                              mut sublen:
                                                  *const libc::c_ushort,
    ...
    let mut cache_available: libc::c_uchar =
        (!(*s).lmc.is_null() &&
            ...
            libc::c_uchar; /* For quitting early. */
        ...
        if *(*(*s).lmc).length.offset(lmcpos as isize) as libc::c_int ==
            ...
            __assert_fail(b"s->lmc->length[lmcpos] == 1 && s->lmc->dist[lmcpos] == 0\x00"
                          ...
                          (*::std::mem::transmute::<&[u8; 122],
                                                    &[libc::c_char; 122]>(b"void StoreInLongestMatchCache(ZopfliBlockState *, size_t, size_t, const unsigned short *, unsigned short, unsigned short)\x00")).as_ptr()); /* Not unsigned short on purpose. */
        };
        *(*(*s).lmc).dist.offset(lmcpos as isize) =
            if (length as libc::c_int) < 3 as libc::c_int {
                0 as libc::c_int
            } else { distance as libc::c_int } as libc::c_ushort;
        *(*(*s).lmc).length.offset(lmcpos as isize) =
            if (length as libc::c_int) < 3 as libc::c_int {
                0 as libc::c_int
            } else { length as libc::c_int } as libc::c_ushort;
        if !(*(*(*s).lmc).length.offset(lmcpos as isize) as libc::c_int ==
                 1 as libc::c_int &&
                 *(*(*s).lmc).dist.offset(lmcpos as isize) as libc::c_int ==
                     0 as libc::c_int) {
...
#[no_mangle]
pub unsafe extern "C" fn ZopfliFindLongestMatch(mut s: *mut ZopfliBlockState,
    ...
    if TryGetFromLongestMatchCache(s, pos, &mut limit, sublen, distance,
                                   length) != 0 {
    ...
    if size.wrapping_sub(pos) < 3 as libc::c_int as libc::c_ulong {
        ...
        *length =
            0 as libc::c_int as
                libc::c_ushort; /* During the whole loop, p == hprev[pp]. */
        *distance = 0 as libc::c_int as libc::c_ushort;
        return
    }
    if pos.wrapping_add(limit) > size { limit = size.wrapping_sub(pos) }
    arrayend =
        (&*array.offset(pos as isize) as
             *const libc::c_uchar).offset(limit as isize);
    ...
    while dist < 32768 as libc::c_int as libc::c_uint {
        let mut currentlength: libc::c_ushort =
            0 as libc::c_int as libc::c_ushort;
        ...
        if dist > 0 as libc::c_int as libc::c_uint {
            ...
            match_0 =
                &*array.offset(pos.wrapping_sub(dist as libc::c_ulong) as
                                   isize) as *const libc::c_uchar;
            /* Testing the byte at position bestlength first, goes slightly faster. */
            if pos.wrapping_add(bestlength as libc::c_ulong) >= size ||
                   *scan.offset(bestlength as libc::c_int as isize) as
                       libc::c_int ==
                       *match_0.offset(bestlength as libc::c_int as isize) as
                           libc::c_int {
                let mut same0: libc::c_ushort =
                    *(*h).same.offset((pos &
                                           (32768 as libc::c_int -
                                                1 as libc::c_int) as
                ...
                if same0 as libc::c_int > 2 as libc::c_int &&
                       *scan as libc::c_int == *match_0 as libc::c_int {
                    let mut same1: libc::c_ushort =
                        *(*h).same.offset((pos.wrapping_sub(dist as
                                                                libc::c_ulong)
                                               &
                                               (32768 as libc::c_int -
                                                    1 as libc::c_int) as
                                                   libc::c_ulong) as isize);
                    let mut same: libc::c_ushort =
                        if (same0 as libc::c_int) < same1 as libc::c_int {
                            same0 as libc::c_int
                        } else { same1 as libc::c_int } as libc::c_ushort;
                ...
                currentlength =
                    scan.wrapping_offset_from(&*array.offset(pos as isize) as
                                                  *const libc::c_uchar) as
                        libc::c_long as libc::c_ushort
                /* The found length. */
            }
            if currentlength as libc::c_int > bestlength as libc::c_int {
                if !sublen.is_null() {
                    ...
                    j =
                        (bestlength as libc::c_int + 1 as libc::c_int) as
                            libc::c_ushort;
                ...
                if currentlength as libc::c_ulong >= limit { break ; }
            }
        }
        ...
        if p as libc::c_int == pp as libc::c_int { break ; }
        dist =
            dist.wrapping_add(if (p as libc::c_int) < pp as libc::c_int {
                                  (pp as libc::c_int) - p as libc::c_int
                              } else {
                                  (32768 as libc::c_int - p as libc::c_int) +
                                      pp as libc::c_int
                              } as libc::c_uint);
        chain_counter -= 1;
        if chain_counter <= 0 as libc::c_int { break ; }
    }
...
*/
#[no_mangle]
pub unsafe extern "C" fn ZopfliLZ77Greedy(mut s: *mut ZopfliBlockState,
    ...
    while i < inend {
        ...
        ZopfliFindLongestMatch(s, h, in_0, i, inend,
                               ...
                               dummysublen.as_mut_ptr(), &mut dist,
                               &mut leng);
        lengthscore =
            GetLengthScore(leng as libc::c_int, dist as libc::c_int);
        /* Lazy matching. */
        prevlengthscore =
            GetLengthScore(prev_length as libc::c_int,
                           prev_match as libc::c_int);
        if match_available != 0 {
            match_available = 0 as libc::c_int;
            if lengthscore > prevlengthscore + 1 as libc::c_int {
                ZopfliStoreLitLenDist(*in_0.offset(i.wrapping_sub(1 as
                                                                      libc::c_int
                                                                      as
                                                                      libc::c_ulong)
                                                       as isize) as
                ...
                } else { current_block_44 = 4567019141635105728; }
            } else {
                ...
                /* Add to output. */
                ZopfliVerifyLenDist(in_0, inend,
                                    i.wrapping_sub(1 as libc::c_int as
                                                       libc::c_ulong), dist,
                                    leng);
                ZopfliStoreLitLenDist(leng, dist,
                                      i.wrapping_sub(1 as libc::c_int as
                                                         libc::c_ulong),
                                      store);
        ...
        match current_block_44 {
            4567019141635105728 => {
                ...
                if lengthscore >= 3 as libc::c_int {
                    ...
                    ZopfliStoreLitLenDist(*in_0.offset(i as isize) as
                                              libc::c_ushort,
                                          0 as libc::c_int as libc::c_ushort,
                                          i, store);
                }
